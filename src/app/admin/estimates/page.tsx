...
export default async function EstimatesPage() {
  const estimates = await prisma.lead.findMany({
    where: { type: "ESTIMATE" },
              ...
              {estimates.length === 0 ? (
                ...
                estimates.map((lead) => (
                  <tr key={lead.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="p-4 text-slate-400 max-w-[200px] truncate" title={lead.selected_features || ""}>{lead.selected_features || "N/A"}</td>
                    <td className="p-4 text-emerald-400 font-mono">
                      {lead.total_cost ? `$${lead.total_cost}` : "N/A"}
                    </td>
                    <td className="p-4 text-slate-400 font-mono text-xs">{lead.created_at.toLocaleDateString()}</td>
                    <td className="p-4">
